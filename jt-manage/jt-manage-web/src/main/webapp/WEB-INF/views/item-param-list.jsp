...
<div id="itemParamEditWindow" class="easyui-window" title="编辑规格参数" data-options="modal:true,closed:true,iconCls:'icon-save',href:'/page/item-param-edit'" style="width:80%;height:80%;padding:10px;">
</div>
<script>
	...
	function formatItemParamData(value , index){
		...
		$.each(json,function(i,e){
			array.push(e.group);		//只获取每个记录中的group元素
		});
		return array.join(",");
	}
    ...
    var itemParamListToolbar = [{
        ...
        handler:function(){
        	...
        	KindEditorUtil.createWindow({
        		url : "/item/param/toEdit?itemParamId="+ids,
        	});
        }
    },{
        text:'删除',
        	    ...
        	    if (r){
                	...
                	$.post("/item/param/delete",params, function(data){
            			if(data.status == 200){
            				...
            				$.messager.alert('提示','删除商品规格失败!'+data.msg,undefined,function(){
            					$("#itemParamList").datagrid("reload");
            				});
            			}
            		});
